...
const fallbackDevices = [
  { id: "main_lights", title: "Main Lights", isOn: true, sliderValue: 80 },
  { id: "board_lights", title: "Board Lights", isOn: true, sliderValue: 100 },
  { id: "projector", title: "Projector", isOn: false, sliderValue: null },
  { id: "hvac", title: "HVAC System", isOn: true, sliderValue: 22 },
  { id: "audio", title: "Audio System", isOn: false, sliderValue: null },
  { id: "emergency_lights", title: "Emergency Lights", isOn: true, sliderValue: 50 },
];

const fallbackStore = new Map(fallbackDevices.map((d) => [d.id, { ...d }]));
...
router.get("/", async (req, res) => {
  try {
    ...
    console.warn("GET /api/devices DB unavailable, using fallback:", err.message);
    res.json({ devices: [...fallbackStore.values()] });
  }
});
...
router.patch("/:id", async (req, res) => {
  try {
    ...
    console.warn("PATCH /api/devices/:id DB unavailable, using fallback:", err.message);

    const current = fallbackStore.get(req.params.id);
    if (!current) {
      return res.status(404).json({ message: "Device not found" });
    }
    ...
    const next = {
      ...current,
      isOn: req.body.isOn ?? current.isOn,
      sliderValue: req.body.sliderValue ?? current.sliderValue,
    };
    fallbackStore.set(req.params.id, next);
